...
class PackedDataset(IterableDataset):
    ...
    def __len__(self):
        ...
        # 计算每个文件包含的块数
        n_blocks_per_file = chunk_size // self._block_size
        ...
        # 计算当前进程能访问的文件数
        max_num_files = len(self._filenames) // num_shards * num_shards
        files_per_shard = max_num_files // num_shards
        
        # 总块数 = 文件数 × 每个文件的块数
        total_blocks = files_per_shard * n_blocks_per_file
        return total_blocks
...
class CombinedDataset(IterableDataset):
    ...
    def __next__(self):
        ...
        return next(self._dataset_iters[idx]), idx
